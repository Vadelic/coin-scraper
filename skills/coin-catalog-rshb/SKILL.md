...
description: Каталог монет Россельхозбанка (coins.rshb.ru) — полный список, поиск конкретной монеты, сводка и аналитика. Use when user asks about РСХБ, Россельхозбанк, RSHB coins, find a coin at RSHB.
...
1. Вызвать `coin-catalog-rshb`.
   ...
   - Поиск на сайте — параметр `query` (`?search_text=` в URL coins.rshb.ru): ключевые слова из запроса пользователя.
