#!/usr/bin/env python3
"""Скрейпер каталога монет ВТБ через BFF API (вариант с Playwright).
...
Запуск: python3 scrape_vtb_coins_playwright.py [options]
Результат: coins_vtb_catalog.json (или путь из --output).

Требуется пакет playwright и установленный Chromium (`playwright install chromium`).
Если нужен режим только со стандартной библиотекой — scrape_vtb_coins.py (HTTP).
"""
...
log = logging.getLogger("vtb_scraper_playwright")
...
async def scrape_vtb_via_playwright(
    ...
    """Загрузка через браузерный fetch из контекста vtb.ru."""
    log.info("Загрузка через Playwright")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Скрейпер каталога монет ВТБ (BFF API), Playwright",
    )
    p.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="итоговый JSON")
    p.add_argument("--timeout", type=float, default=DEFAULT_TIMEOUT, help="не используется")
...
async def main_async(argv: list[str] | None = None) -> int:
    ...
    log.info("=" * 60)
    log.info("  Скрейпер каталога монет ВТБ (BFF), Playwright")
    log.info("=" * 60)
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        logging.getLogger("vtb_scraper_playwright").error(str(e))
        return 2
