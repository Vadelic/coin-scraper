...
class SberbankLiveTest extends AbstractLiveScraperTest {
    ...
    void pobedonosetsInvestmentCatalog() throws Exception {
        scrapeAndAssert(ScrapeSource.SBERBANK, ScrapeRequest.of("победоносец", true, null));
    }

    @Test
    void georgiyQueryIncludesSilverPobedonosets() throws Exception {
        var result = scrapeRegistry.run(ScrapeSource.SBERBANK, ScrapeRequest.of("георгий победоносец", true, null));
        ScraperTestSupport.assertLiveResult(result, ScrapeSource.SBERBANK);
